...
fs.writeFileSync("Air.json", JSON.stringify({address: airContractAddr, code: airCode, abi: Air.abi}))
console.log("Air.json exported")

// only needed for testnet deployment
let earthCode = Earth.deployedBytecode;
earthCode = replaceAll(earthCode, "1231111111111111111111111111111111111123", testnetCO2);
earthCode = replaceAll(earthCode, "2341111111111111111111111111111111111234", testnetGOL);
earthCode = replaceAll(
  earthCode,
  "4561111111111111111111111111111111111456",
  airContractAddr
);
earthCode = replaceAll(earthCode, "5671111111111111111111111111111111111567", gameMaster);
const earthContractAddr = bufferToHex(ripemd160(earthCode));
fs.writeFileSync("Earth.json", JSON.stringify({address: earthContractAddr, code: earthCode, abi: Earth.abi}))
console.log("Earth.json exported")
